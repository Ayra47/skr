@php
    $lifetimeOptions = [
        \App\Models\FeedPost::EXPIRES_1H      => '1 ч',
        \App\Models\FeedPost::EXPIRES_24H     => '24 ч',
        \App\Models\FeedPost::EXPIRES_7D      => '7 дн',
        \App\Models\FeedPost::EXPIRES_FOREVER => '∞',
    ];
...
<div class="fd-content">
    ...
    @if($tab === 'groups')
        <section class="fd-community-search" aria-label="Поиск в сообществах">
            <input
                type="search"
    ...
    {{-- Composer --}}
    <section class="feed-composer" id="fd-composer" aria-label="Создать пост">
        <form class="feed-composer-form" wire:submit="createPost" enctype="multipart/form-data">
            <div class="feed-composer-row">
                ...
                <div class="feed-composer-main">
                    <textarea class="feed-textarea" wire:model="body" rows="2" maxlength="2000" placeholder="Поделитесь чем-то с друзьями…" data-feed-textarea></textarea>
                    ...
                    <div class="feed-composer-actions">
                        <label class="feed-file-button">
                            ...
                            <span>
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
                                Фото
                            </span>
                        </label>
                        ...
                        <label class="feed-file-button">
                            <input type="file" wire:model="attachments" data-feed-attachment multiple accept=".jpg,.jpeg,.png,.gif,.webp,.mp4,.webm,.mov,.pdf,.txt,.md,.zip">
                            <span>
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/></svg>
                                Файл
                            </span>
                        </label>
                        ...
                        <button class="feed-poll-toggle {{ $hasPoll ? 'active' : '' }}" type="button" wire:click="togglePoll" aria-pressed="{{ $hasPoll ? 'true' : 'false' }}" title="Добавить опрос">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="13" width="4" height="8" rx="1"/><rect x="10" y="8" width="4" height="13" rx="1"/><rect x="17" y="3" width="4" height="18" rx="1"/></svg>
                            Опрос
                        </button>
                        ...
                        <button class="feed-whisper-toggle {{ $isWhisper ? 'active' : '' }}" type="button" data-feed-whisper-toggle aria-pressed="{{ $isWhisper ? 'true' : 'false' }}" title="Тихий пост — без псевдонима">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
                            Тихо
                        </button>
                        ...
                        <button class="feed-submit" type="submit" data-feed-submit wire:loading.attr="disabled" wire:target="createPost,attachments" disabled>
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M22 2 11 13"/><path d="m22 2-7 20-4-9-9-4 20-7z"/></svg>
                            Опубликовать
                        </button>
                    </div>

                    <div class="feed-whisper-hint" data-feed-whisper-hint @if(! $isWhisper) hidden @endif>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="10" width="16" height="11" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg>
                    ...
                    @if($hasPoll)
                        <div class="feed-poll-composer">
                            ...
                            <div class="feed-poll-composer-options">
                                @foreach($pollOptions as $i => $optionText)
                                    <div class="feed-poll-composer-option-row">
                                        <input class="feed-poll-composer-option-input" type="text" wire:model="pollOptions.{{ $i }}" maxlength="100" placeholder="Вариант {{ $i + 1 }}">
                            ...
                            <div class="feed-poll-composer-settings">
                                <div class="feed-poll-composer-mode" role="radiogroup">
                                ...
                                @if($pollMode === \App\Models\Poll::MODE_MULTIPLE)
                                    <div class="feed-poll-composer-max">
                                        <label><span>Макс. вариантов</span><input type="number" wire:model="pollMaxChoices" min="2" max="10" placeholder="любое" class="feed-poll-composer-max-input"></label>
                                    </div>
                                @endif
                                <div class="feed-poll-composer-expires" role="radiogroup">
                                    @foreach(['12h' => '12ч', '24h' => '24ч', '7d' => '7д', 'never' => '∞'] as $val => $lbl)
                                        <label class="{{ $pollClosesIn === $val ? 'active' : '' }}">
                                            <input type="radio" wire:model="pollClosesIn" value="{{ $val }}" hidden>{{ $lbl }}
                                        </label>
                                    @endforeach
                                    <label class="{{ $pollClosesIn === 'custom' ? 'active' : '' }}">
                                        <input type="radio" wire:model="pollClosesIn" value="custom" hidden>Дата
                                    </label>
                                </div>
    ...
    <div class="fd-posts" aria-label="Посты">
    ...
    {{-- Post modal --}}
    @if($modalPost)
        @php $modalAuthorFeedName = $modalPost->author?->feedName(); @endphp
